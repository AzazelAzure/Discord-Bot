...
def game_init(channel, guild, message=None, player=None, *args):
    ...
    if ready_check == True:
        ...
        if message == '!next':
            with open(filename) as f:
                gamers = f.readlines()
                print(f'Gamers duude: {gamers}')
            for g in gamers:
                print(f'G in gamers: {g}')
                filename = f'Alibis/games/{guild}{channel}{g.rstrip()}.txt'
                try:
                    with open(filename):
                        print(f'G: {g}, Path open, filename: {filename}')
                        investigation(channel, guild, players=g)
                except FileNotFoundError:
                    with open(filename, 'w') as f:
                        print(f'G: {g} Path FNF, filename:{filename}')
                        f.write(g)
                        continue
...
def investigation(channel, guild, players='newgame'):
    ...
    if len(gamers) > 1:
        try:
            ...
            print(f'Gamers: {gamers}, Order: {order}, FUCK!')
