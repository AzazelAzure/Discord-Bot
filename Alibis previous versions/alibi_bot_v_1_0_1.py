...
def game_init(channel, guild, message=None, player=None, *args):
    ...
    if ready_check == True:
        ...
        if message == '!next':
            ...
            if play[0] == '':
                return False
            player = play[randint(0,(len(play)))]
            with open(filename, 'a') as f:
                f.replace(player, '')
...
class MyClient(discord.Client):
    ...
    async def on_message(self, message):   
        ...
        if message.content.startswith('!join'):
            logging.BASIC_FORMAT(f'Caller: {message.author} line:246')
        ...
        if message.content.startswith('!start'):  
            ...
            if game == True:
                ...
                logging.BASIC_FORMAT(f'Judge: {judge}')
                killer = players[randint(0,(len(players)-1))]
                logging.BASIC_FORMAT(f'Killer: {killer}')
        ...
        if message.content.startswith('!next'):
            ...
            if game == True:
                ...
                converter = MemberConverter()
                if n_player == False:
                    await message.channel.send(f'All players have had their turn, quitting')
